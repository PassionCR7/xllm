...
def train_epoch(epoch,loader,iters,start_step=0,wandb=None):
    ...
    for step,(input_ids,labels,attention_mask) in enumerate(loader,start=start_step+1):
        ...
        if step%args.accumulation_steps==0:
            scaler.unscale_(optimizer) #在梯度剪切前取消缩放
            torch.nn.utils.clip_grad_norm_(model.parameters(),args.grad_clip)
            scaler.step(optimizer) #更新参数
            scaler.update() #更新缩放器
            optimizer.zero_grad(set_to_none=True) #清0梯度
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lm_checkpoint(
                ...
                step=step,
                wandb=wandb,
                save_dir=os.path.join(ROOT, "checkpoints"),  # ！修正：改为基于项目根目录的绝对路径
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XLLM Pretraining")

    # ========== 基础训练参数 ==========
    parser.add_argument(
        "--save_dir", type=str, default=os.path.join(ROOT, "out"), help="模型保存目录"
    )  # ！修正：原"../out"相对 cwd 易出错，改为基于项目根目录的绝对路径
    parser.add_argument(
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="batch size（8GB显存实测最大稳定值；梯度累积8步→有效批量128）")
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default=os.path.join(  # ！修正：原"../dataset"相对 cwd 易出错，改为基于项目根目录的绝对路径
            ROOT, "dataset", "pretrain_t2t_mini.jsonl"
        ),
        help="预训练数据路径",
    )
    parser.add_argument(
    ...
    ckp_data = (
        lm_checkpoint(
            lm_config, weight=args.save_weight, save_dir=os.path.join(ROOT, "checkpoints")
        )  # ！修正：改为基于项目根目录的绝对路径
