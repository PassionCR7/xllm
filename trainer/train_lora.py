# 当前这份 LoRA 实现的局限
# 只支持方阵 Linear 层
# apply_lora条件：module.weight.shape[0]==module.weight.shape[1]
# 只对输入输出维度相等的线性层添加 LoRA；
# Transformer 里的q_proj、k_proj、v_proj很多实现是分开的非方阵，本代码不会给它们加 LoRA；
# 只会作用于o_proj/FFN 中输入输出相同维度的层。
# 现实主流 LoRA 一般作用于 QKV 投影矩阵，这份实现适配范围受限。
#无法指定只给某几层 transformer 层开启 LoRA；深层 / 浅层不能单独控制，会引入不必要参数


#方便切换多个 LoRA
#默认 rank=8，没有从命令行参数读取 rank；想要修改秩要改代码，不方便多实验调参
"""
XLLM LoRA (Low-Rank Adaptation) 微调脚本  
...
# XLLM相关组件
from model.XLLMModel import XLLMConfig  # 模型配置
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="XLLM LoRA Fine-tuning")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="XLLM-LoRA", help="wandb项目名"
    )
    ...
    # 📚 模型配置初始化
    # XLLMConfig: 定义模型的超参数，如隐藏维度、层数、是否使用MoE
    ...
    if args.use_wandb and is_main_process():
        ...
        # 📚 实验名称生成
        # 包含关键参数，便于识别不同的实验配置
        wandb_run_name = f"XLLM-LoRA-{args.lora_name}-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
...
# 初始化DDP分布式环境 + 设置随机种子
#     ↓
# 读取/创建模型配置 MokioMindConfig
#     ↓
# 检测断点 ckp_data（from_resume）
#     ↓
# 配置混合精度上下文 autocast_ctx
#     ↓
# 初始化实验跟踪工具(仅主进程)
#     ↓
# 加载预训练模型 init_model()
#     ↓
# apply_lora(model) 给方阵Linear注入LoRA旁路层
#     ↓
# 参数冻结：仅 lora.* 参数 requires_grad=True，其余全部冻结
#     ↓
# 构建 SFT数据集 + DataLoader（DistributedSampler多卡）
#     ↓
# 初始化 GradScaler + AdamW(只传lora_params)
#     ↓
# 如果存在断点：恢复 model / optimizer / scaler / start_epoch / start_step
#     ↓
# DDP 包装 model（忽略旋转位置编码buffer）
#     ↓
# for epoch in range(start_epoch, args.epochs):
#     ├─ 分布式采样器 set_epoch(epoch)
#     ├─ if 断点续训且是恢复的第一个epoch：
#     │    使用 SkipBatchSampler 跳过已经训练过的batch
#     ├─ else：正常构造DataLoader
#     └─ 调用 train_epoch()
#            ├─ for 每个batch(input_ids,labels,attention_mask):
#            │    ├─ 数据to(device)
#            │    ├─ 更新余弦退火学习率
#            │    ├─ with autocast_ctx: 前向传播，计算 loss = loss+aux_loss
#            │    ├─ loss /= accumulation_steps
#            │    ├─ scaler.scale(loss).backward() 反向传播
#            │    ├─ if 到达梯度累积步数：
#            │    │    scaler.unscale_ → clip_grad_norm_(lora_params) → scaler.step → scaler.update → zero_grad
#            │    ├─ if 日志间隔到达：打印loss、lr、ETA，wandb记录
#            │    └─ if 保存间隔到达 && 主进程：
#            │           model.eval() → save_lora(只存A/B矩阵) → lm_checkpoint完整断点 → model.train()
