...
class InternLM2Attention(nn.Module):
    ...
    def _init_rope(self):
        ...
        # 兼容新版 transformers：配置加载时会把 rope_scaling=null 规范化为
        # {'rope_type': 'default', ...}（无旧格式 'type'/'factor' 键），
        # 'default'/None 表示不启用缩放，按无缩放处理
...
class InternLM2Model(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        # 兼容新版 transformers（v5 已移除 DynamicCache.from_legacy_cache）：
        # 本模型用于奖励打分，前向从不传 past_key_values；只有当确有旧格式缓存时才转换，
        # None（无缓存）时直接跳过，避免调用已删除的 API 触发 AttributeError
        if (
            use_cache
            and past_key_values is not None
            and not isinstance(past_key_values, Cache)  # kept for BC (non `Cache` `past_key_values` inputs)
        ):
...
class InternLM2ForRewardModel(InternLM2PreTrainedModel):
    ...
    def _remap_oob_ids(self, tokenizer, input_ids):
        """
        兼容新版 transformers（v5）兼容补丁。

        背景：旧版 InternLM2 远程 tokenizer 在 transformers v5 下加载时，
        v5 会忽略 tokenizer_config.json 里保存的 added_tokens_decoder 的 id 映射，
        将 <|im_start|>、<|im_end|>、<|reward|> 等 7 个附加特殊 token 追加到词表末尾
        （id >= config.vocab_size），而模型权重训练时这些 token 位于词表内的占位 id 上
        （例如 <|reward|> -> 92527、<|im_start|> -> 92543）。这会导致 embedding 越界。

        本函数把编码结果中越界的特殊 token id 映射回词表内的正确 id。
        正常情况（无越界 id 或读取不到映射表）下不做任何改动。
        """
        ...
        if int(input_ids.max()) < vocab_size:
            return input_ids
        # 读取 tokenizer_config.json 中保存的 added_tokens_decoder：内容 -> 词表内 id
        saved = {}
        base_path = getattr(tokenizer, "name_or_path", None) or getattr(self.config, "_name_or_path", None)
        if base_path:
            cfg_path = os.path.join(str(base_path), "tokenizer_config.json")
            if os.path.isfile(cfg_path):
                try:
                    with open(cfg_path, encoding="utf-8") as f:
                        atd = json.load(f).get("added_tokens_decoder", {}) or {}
                    saved = {
                        str(v["content"]): int(k)
                    ...
                    saved = {}
        # 对每个特殊 token：当前 id（可能越界） -> 保存 id（词表内）
        remap = {}
        special_tokens = getattr(tokenizer, "all_special_tokens", None) or []
        for tok in special_tokens:
            cur = tokenizer.convert_tokens_to_ids(tok)
            if isinstance(cur, int) and cur >= vocab_size:
                saved_id = saved.get(str(tok))
                if saved_id is not None and saved_id < vocab_size:
                    remap[cur] = saved_id
        if not remap:
            return input_ids
        input_ids = input_ids.clone()
        for oob_id, ok_id in remap.items():
            input_ids[input_ids == oob_id] = ok_id
        return input_ids
    ...
    def get_score(
        ...
        **kwargs,
        ...
        # 兼容 transformers v5：把 v5 追加到词表末尾的越界特殊 token id 映射回词表内
        input_ids = self._remap_oob_ids(tokenizer, input_ids)
    ...
    def get_scores(
        ...
        **kwargs,
        ...
        for conversation_str in conversation_strs:
            input_ids = tokenizer.encode(conversation_str, return_tensors="pt", add_special_tokens=False)
            # 兼容 transformers v5：把越界特殊 token id 映射回词表内
            input_ids = self._remap_oob_ids(tokenizer, input_ids)
            # add reward score token at the end of the input_ids if it is not already there
            if input_ids[0, -1] != self.reward_token_id:
                input_ids = torch.cat([input_ids, torch.tensor([[self.reward_token_id]], dtype=torch.long)], dim=1)
