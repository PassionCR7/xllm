...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        # 📚 模型定期保存权重 + 完整断点checkpoint
        #保存模型、优化器、scaler、epoch、step，用于断点续训from_resume=1
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            model.eval()  # 切换到评估模式（禁用dropout等）

            # 构建保存路径（根据是否使用MoE添加后缀）
            moe_suffix = "_moe" if lm_config.use_moe else ""
            ckp = f"{args.save_dir}/{effective_save_weight}_{lm_config.hidden_size}{moe_suffix}.pth"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XLLM Full SFT")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="XLLM-Full-SFT", help="wandb项目名"
    )
    parser.add_argument(
        "--use_compile",
        default=0,
    ...
    type=int,
    choices=[0, 1],
    help="是否使用reasoning_content做CoT训练；0关闭(丢弃reasoning_content),1开启(拼接块)"
)

    args = parser.parse_args()

    # 动态拼接cot后缀，区分是否开启CoT训练
    effective_save_weight = args.save_weight
    if args.use_cot == 1:
        effective_save_weight += "_cot"
    ...
    # 尝试加载断点续训数据
    ckp_data = (
        lm_checkpoint(lm_config, weight=effective_save_weight, save_dir="../checkpoints")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"XLLM-Full-SFT-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
        wandb.init(
    ...
    # 加载SFT数据集 #增加是否启用CoT
    train_ds = SFTDataset(args.data_path, tokenizer, max_length=args.max_seq_len,use_cot=args.use_cot)
    # 分布式采样器：确保不同进程训练不同数据
...
#     ├ pre_processing_chat 随机插入system prompt
#     ├ create_chat_prompt apply_chat_template渲染对话字符串
#     ├ post_processing_chat清理空think块
#     ├ tokenize、truncate、pad补齐max_length
#     └ generate_labels()生成稀疏labels，仅assistant有效
#     ↓
# 返回 (input_ids, labels, attention_mask)
#     ↓
# DataLoader → batch → to GPU
#     ↓
# res = model(input_ids, labels=labels, attention_mask=attention_mask)
#     ↓ 模型内部：使用labels=-100 ignore_index，只计算assistant部分loss
# loss = res.loss + res.aux_loss
#     ↓
# loss / accumulation_steps → backward反向传播
#     ↓
# 梯度累积满足条件 → unscale → grad clip → optimizer.step → zero_grad
#     ↓
# log打印，定期保存权重&checkpoint

#python train_full_sft.py --use_moe=1 --use_cot=1 